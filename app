...
export default function Home() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // Appelle le script Python via l'API
    try {
      const response = await fetch('/api/run-script', {
        ...
        body: JSON.stringify({ mot: searchTerm }),
      });

      const data = await response.json();
      console.log('Résultat Python:', data.result); // Tu peux utiliser ce résultat comme tu veux
      ...
      console.error('Erreur lors de l’appel au script Python:', error);
    }
  };
